...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `Wallet Address: ${walletAddress}, EigenPod Address: ${eigenPodAddress} added to database`
    );

    // Validate the input
    if (!walletAddress || !eigenPodAddress) {
      return NextResponse.json(
        { error: "Wallet address and EigenPod address are required" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: "Wallet and EigenPod addresses added successfully" },
      { status: 200 }
    );
  } catch (error) {
